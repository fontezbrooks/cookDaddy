#!/usr/bin/env bash
# Cloud-supabase pgTAP dry-run for B2 atomic recipe graph writer (migration 020 + test).
# Savepoint-munging skill: one outer tx -> apply migration 020 DDL -> install
...

{
  ...
  cat supabase/migrations/020_import_recipe_graph.sql
